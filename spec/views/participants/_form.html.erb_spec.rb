...
describe "participants/_form.html.erb" do
  ...
  let(:participant) { stub_model(Participant, user_account: user, crew: crews[2], last_name: 'Smith',
  ...
  describe "for existing participant" do
    ...
    it { should have_select('participant[crew_id]') }
    it { should have_selector('option', count: 4) }
    it { should have_selector("option[selected='selected']", count: 1, text: crews[2].name) }
